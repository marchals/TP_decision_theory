...
compute_cautious_prediction <- function(){
  mat_risk <- c()
  for(ex in 1:nrow(M2)){
    cautious_prediction <- c()
    for (a in 1:nrow(LOSS)){
      cautious_prediction <- c(cautious_prediction, risk(a, M2[ex,]))
    }
...
cautious_prediction_matrix <- compute_cautious_prediction()
...
compute_predictions <- function(){
  ...
  for(i in 1:nrow(cautious_prediction_matrix)){
    ...
    cautious_predictions <- c(predictions, unname(which.min(line)))
  }
  cautious_predictions
}
cautious_predictions <- compute_predictions()
cautious_predictions
```
