...
compute_cautious_prediction <- function(mat_loss, M){
  mat_risk <- c()
  for(ex in 1:nrow(M)){
    cautious_prediction <- c()
    for (a in 1:nrow(mat_loss)){
      cautious_prediction <- c(cautious_prediction, risk(a, M[ex,], mat_loss))
    }
    mat_risk <- c(mat_risk, cautious_prediction)
  }
  mat_risk <- matrix(mat_risk, nrow(M), ncol(M), TRUE)
  return(as.data.frame(mat_risk))
}

compute_predictions <- function(mat_loss,M){
  cautious_predictions <- c()
  cautious_prediction_matrix <- compute_cautious_prediction(mat_loss, M)
...
```{r}
disjonctive.comb<- function(m1, m2){
  n1 = length(m1)
  n2 = length(m2)
  grand_theta = max(n1,n2)
  m1u2 = c(rep(0,grand_theta))
  for(k in 1:grand_theta){
    somme_k = 0
    for (i in 1:n1){
      for (j in 1:n2){
...
m1 = c(0.5, 0, 0, 0.4, 0, 0, 0.1)
m2 = c(0, 0, 0, 0.4, 0.6, 0, 0)
disjonctive.comb(m1, m2)
```

Q 5.3)

```{r}
nb_exemples = length(M1[,1])
nb_sous_ensembles = 31
M12conj <- matrix(rep(c(0),nb_exemples*nb_sous_ensembles), nb_exemples,nb_sous_ensembles)
M12disj <- matrix(rep(c(0),nb_exemples*nb_sous_ensembles), nb_exemples,nb_sous_ensembles)
for (i in 1:nb_exemples){
  M1_natural_order = c(rep(0,31))
  M1_natural_order[1] = M1[i,1]
  M1_natural_order[2] = M1[i,2]
  M1_natural_order[4] = M1[i,3]
  M1_natural_order[8] = M1[i,4]
  M1_natural_order[16] = M1[i,5]
  M1_natural_order[31] = M1[i,6]
  M12conj[i,] = dempster.comb(M1_natural_order, M2[i,])
  M12disj[i,] = disjonctive.comb(M1_natural_order, M2[i,])
}
```
...
{
  mat_loss = mat_loss_init(beta = beta)
  predictions = compute_predictions(mat_loss, M1_natural_order)
  list_corr[beta] = av_corr(predictions, test_labels_number)
  list_rel[beta] = av_rel(predictions, test_labels_number)
  print(paste('les calculs pour beta =',beta,'sont effectués'))
}
...
{
  mat_loss = mat_loss_init(beta = beta)
  predictions = compute_predictions(mat_loss, M12conj)
  list_corr[beta] = av_corr(predictions, test_labels_number)
  list_rel[beta] = av_rel(predictions, test_labels_number)
  print(paste('les calculs pour beta =',beta,'sont effectués'))
}
...
{
  mat_loss = mat_loss_init(beta = beta)
  predictions = compute_predictions(mat_loss, M12disj)
  list_corr[beta] = av_corr(predictions, test_labels_number)
  list_rel[beta] = av_rel(predictions, test_labels_number)
  print(paste('les calculs pour beta =',beta,'sont effectués'))
}
